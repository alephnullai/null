...
**Linux note (Debian/Ubuntu):** modern distros ship an *externally managed*
system Python ([PEP 668](https://peps.python.org/pep-0668/)), so a bare
`pip install` into the system Python is refused. Install into a virtual
environment instead (recommended on every OS):
```bash
sudo apt install python3-venv    # Debian/Ubuntu only: needed for `python -m venv`
python3 -m venv .venv
source .venv/bin/activate
pip install null-memory
```

> **Make `null` reachable.** In a venv, the `null` command lives in
> `.venv/bin` and is on your PATH only while the venv is **activated**. The
> capture hooks and several docs call bare `null`, so if it isn't on PATH the
> hooks silently no-op. Either keep the venv activated, symlink it onto your
> PATH (`ln -s "$(pwd)/.venv/bin/null" ~/.local/bin/null`), or reference the
> full interpreter path in your MCP config (see Setup).

Or from source, for a pinned/reviewed copy (see "Production vs development"):
```bash
...
```bash
# First run — name your agent and give it a personality (interactive)
null init

# ...or just set the name directly
null name Sage
