...
def _write_json_atomic(path: str, obj: dict) -> None:
    ...
    mid-write can never leave a truncated/corrupt file behind (issue #42).
    Same-directory matters: os.replace is only atomic within a volume."""
    tmp = f"{path}.tmp"
...
def save_state(state: dict, agent_dir: str | None = None) -> None:
    ...
    _write_json_atomic(_state_path(agent_dir), state)
...
def save_momentum(momentum: dict, agent_dir: str | None = None) -> None:
    ...
    _write_json_atomic(_momentum_path(agent_dir), momentum)
