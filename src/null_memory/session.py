...
class MemoryRepo:
    ...
    def commit(self, message: str, allow_empty: bool = False) -> bool:
        ...
        Retries up to 3 times if git index.lock blocks the operation
        (can happen when background sync thread is mid-commit).

        Lazily initializes the repo if it does not exist yet. Session start
        no longer inits the repo on the foreground request path; the first
        commit (always on a background sync/lifecycle thread) is what brings
        the repo into being. ``init`` is idempotent + lock-guarded, so this
        is safe even if the deferred session-start git thread is racing it.
        """
        if not self.is_repo():
            self.init()
            if not self.is_repo():
        ...
        for attempt in range(3):
            # Check for index.lock and wait if present
            lock_path = os.path.join(self.repo_dir, ".git", "index.lock")
            if os.path.exists(lock_path) and attempt < 2:
                time.sleep(0.5)
                continue
