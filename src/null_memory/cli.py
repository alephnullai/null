...
def main() -> None:
    ...
    # init — first-run identity wizard (name + a few personality traits)
    init_parser = subparsers.add_parser(
        "init", help="Name your agent and set its personality (first-run wizard)")
    init_parser.add_argument(
        "--no-input", action="store_true",
        help="Don't prompt; keep the generated name (for scripts/CI)")
    ...
    if args.command == "serve":
        ...
        _run_init_wizard(getattr(args, "no_input", False))
        ...
        # Install the bundled /doorbell skill so a fresh seat can relay work
        # over the org exchange out of the box (idempotent; never clobbers a
        # user-edited copy).
        _install_bundled_skill("doorbell")
...
def _run_init_wizard(no_input: bool = False) -> None:
    """First-run identity wizard: name the agent + a few personality traits.

    Interactive only. On a non-TTY, in CI, or with --no-input, it does NOT
    prompt (a seat or a scripted install must never hang on input) — it keeps
    the already-generated random name and points the user at `null init`.
    'atlas' is reserved (Pete's persona) and rejected.
    """
    import random
    from null_memory.agent import AgentMemory, _AGENT_NAMES

    mem = AgentMemory.load()
    current = mem.identity.get("name", "") or "your agent"

    non_interactive = (
        no_input
        or os.environ.get("NULL_NONINTERACTIVE")
        or os.environ.get("CI")
        or not sys.stdin.isatty()
    )
    if non_interactive:
        # Persist the current name so it's STABLE. A fresh store generates a
        # random name in memory but never saves it, so without this the name
        # would change on every load. (A seat re-saves its real name — a no-op.)
        mem.save_identity()
        print(f"[null] Non-interactive — keeping the name '{current}'. "
              "Run `null init` in a terminal to name your agent and set its "
              "personality.")
        return

    picks = random.sample([n for n in _AGENT_NAMES if n.lower() != "atlas"], 3)
    print("Welcome to Null. Let's give your agent an identity — this is who it")
    print("will be across every session and machine.\n")

    # 1. Name
    while True:
        raw = input(
            f"  What should it be called? [{', '.join(picks)}, or your own] "
            f"(Enter for {picks[0]}): "
        ).strip()
        name = raw or picks[0]
        if name.lower() == "atlas":
            print("    'Atlas' is reserved — please pick another name.")
            continue
        break

    # 2. A few traits — comma-separated, all optional.
    print("\n  A few traits shape how it works together with you.")
    print("  Comma-separated, press Enter to skip any.\n")
    dos = input(
        "  Do's — how should it work? "
        "(e.g. push back on bad ideas, cite sources, explain trade-offs): "
    ).strip()
    donts = input(
        "  Don'ts — what should it avoid? "
        "(e.g. no filler, don't guess silently, no emojis): "
    ).strip()

    caps = [d.strip() for d in dos.split(",") if d.strip()]
    antis = [d.strip() for d in donts.split(",") if d.strip()]

    mem.identity["name"] = name
    if caps:
        mem.identity["capabilities"] = caps
    if antis:
        mem.identity["anti_patterns"] = antis
    mem.save_identity()

    print(f"\n✓ Your agent is {name}. Identity saved to {mem.agent_dir}.")
    if caps:
        print(f"  Do's:   {', '.join(caps)}")
    if antis:
        print(f"  Don'ts: {', '.join(antis)}")
    print("  Change it any time with `null init` or `null name <name>`.")


def _null_scripts_dir() -> str | None:
    """Locate the directory holding the null-*-hook.py capture hooks.

    The hooks ship as package data at ``null_memory/hooks/``, so this
    resolves for BOTH wheel and editable installs — that's the fix for
    wheel installs that used to omit the scripts entirely (#53/#68).
    Falls back to a legacy repo-root ``scripts/`` checkout for old layouts.
    Returns None if neither is present (callers fail-soft)."""
    pkg_dir = os.path.dirname(os.path.abspath(__file__))      # …/null_memory
    hooks_dir = os.path.join(pkg_dir, "hooks")
    if os.path.isdir(hooks_dir) and any(
        n.endswith("-hook.py") for n in os.listdir(hooks_dir)
    ):
        return hooks_dir
    # Legacy: hooks used to live at the repo-root scripts/ dir.
    repo_root = os.path.dirname(os.path.dirname(pkg_dir))     # …/
    legacy = os.path.join(repo_root, "scripts")
    if os.path.isdir(legacy) and any(
        n.endswith("-hook.py") for n in os.listdir(legacy)
    ):
        return legacy
    return None
...
def _null_bundled_skill_dir(name: str) -> str | None:
    """Locate a bundled Claude Code skill (e.g. 'doorbell').

    Skills ship as package data at ``null_memory/skills/<name>/``, so this
    resolves for BOTH wheel and editable installs. Falls back to a repo-root
    ``skills/<name>/`` checkout for old layouts. Returns None if neither has a
    SKILL.md (callers fail-soft)."""
    pkg_dir = os.path.dirname(os.path.abspath(__file__))      # …/null_memory
    cand = os.path.join(pkg_dir, "skills", name)
    if os.path.isfile(os.path.join(cand, "SKILL.md")):
        return cand
    repo_root = os.path.dirname(os.path.dirname(pkg_dir))     # …/
    legacy = os.path.join(repo_root, "skills", name)
    if os.path.isfile(os.path.join(legacy, "SKILL.md")):
        return legacy
    return None


def _install_bundled_skill(name: str) -> bool:
    """Install a bundled skill into ``~/.claude/skills/<name>/`` so it works as
    a Claude Code slash-command out of the box. Non-destructive: if the user
    already has a SKILL.md there it is left untouched (never clobber local
    edits). Returns True if the skill is present afterward."""
    import shutil
    src = _null_bundled_skill_dir(name)
    dest = os.path.join(os.path.expanduser("~"), ".claude", "skills", name)
    dest_skill = os.path.join(dest, "SKILL.md")
    if os.path.isfile(dest_skill):
        print(f"[null] /{name} skill already present at {dest} (left as-is).")
        return True
    if src is None:
        print(f"[null] bundled skill '{name}' not found (packaged install "
              "missing skills data); skipping.")
        return False
    os.makedirs(dest, exist_ok=True)
    for fn in os.listdir(src):
        if fn.endswith(".md"):
            shutil.copy2(os.path.join(src, fn), os.path.join(dest, fn))
    print(f"[null] Installed /{name} skill → {dest}")
    return os.path.isfile(dest_skill)
