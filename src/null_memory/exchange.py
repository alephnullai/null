...
# Word forms peers actually put in `confidence` — a seat on a different model
# (or a human-authored event) sends "high" where the schema expects a float.
_CONFIDENCE_WORDS = {
    "certain": 0.99, "high": 0.9, "moderate": 0.6, "medium": 0.6,
    "med": 0.6, "low": 0.3, "none": 0.05,
}


def _coerce_confidence(val: object, default: float) -> float:
    """Coerce an event's confidence to a float, tolerating the string levels
    peers send. ``float("high")`` raises ValueError, and because that raise
    was uncaught inside `_ingest_fact`, it aborted `_apply_foreign_event`
    before the view event applied — silently dropping the ENTIRE foreign
    event (fact and message). Found by Talos on 2026-07-14: Prometheus was
    tripping it and ~5 cross-seat events were lost. Map known words; fall
    back to ``default`` on anything unparseable, never raise."""
...
class ExchangeClient:
    ...
    def _ingest_fact(self, ev: dict, writer: str) -> None:
        ...
        kind = ev.get("kind", "")
        discount = _coerce_confidence(
            self.config.get("confidence_discount"), DEFAULT_CONFIDENCE_DISCOUNT)
        base = _coerce_confidence(
            data.get("confidence"), DEFAULT_INGEST_CONFIDENCE)
