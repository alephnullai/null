...
class IdentityPayload:
    ...
    role: str
    focus: str
    # The agent's own display name (from identity.json). Distinct from
    # `personality` (the store/attribution key): a fresh install has
    # personality "atlas" by back-compat default but a generated name like
    # "Onyx" — the boot prompt must say the NAME, never assert it's Atlas.
    name: str = ""
...
def _render(p: IdentityPayload) -> str:
    ...
    # Identity template parameterized by the agent's own NAME — never a
    # hardcoded persona. A fresh install (personality "atlas" by back-compat
    # default, but a generated name) must NOT be told "You are Atlas", and no
    # shipped template may assert a specific person's descriptor ("Pete's AI
    # technical lead" lives in Pete's own anchors, not here). Each agent's
    # voice grows from its own anchors/decisions.
    display = (p.name or p.personality or "agent").strip()
    header = f"═══ {display.upper()} IDENTITY ═══"
    intro = f"You are {display[:1].upper() + display[1:]}."
    lines = [
        header,
        "",
        intro,
...
def build_identity_payload(
    ...
    ``name`` is the agent's display name (from identity.json); when omitted
    it falls back to the personality (back-compat: an atlas store renders
    "You are Atlas"). Callers should pass the store's real name so a fresh
    install introduces itself as ITS agent, not as Atlas.
...
def render_identity_markdown(payload: IdentityPayload) -> str:
    ...
    if payload.anchors:
        for a in payload.anchors:
            ...
            if atype == "code_word":
                # SECURITY: same rule the Code Word section above enforces
                # — a code_word anchor's fact text IS the code word. The
                # first live leak shipped the full phrase through this
                # loop, eleven lines below the comment forbidding it (the
                # leaked text even contained the prohibition). Verification
                # is covered by the fingerprint section; render only a
                # redaction marker here.
                lines.append(
                    "- **[code_word]** (redacted — see the Code Word "
                    "fingerprint above; the phrase lives only in the "
                    "database)")
                continue
