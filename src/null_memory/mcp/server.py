...
async def _watchdog_call(handlers: NullHandlers, tool: str, fn) -> str:
    ...
    # GetTickCount64() with a 15.625ms resolution, so any call that finishes
    # inside one tick measures as EXACTLY 0.0 elapsed — and `elapsed > soft`
    # is then False, so the violation breadcrumb is never recorded. (POSIX
    # monotonic is nanosecond-grade, which is why this was invisible there.)
    # perf_counter is the highest-resolution monotonic clock and is what the
    # rest of the codebase already uses for interval timing (selftest).
...
def instructions_for_personality(name: str) -> str:
    """Base MCP instructions parameterized by the agent's DISPLAY NAME.

    The first line of SYSTEM_INSTRUCTIONS names the agent. Keying on the
    *personality* was wrong: a fresh pip install has personality 'atlas' by
    back-compat default but a generated display name (Sage/Onyx/…), so the
    cold-start prompt still said "You are Atlas" (H6 — Talos's catch, and the
    residual that compounds when a brand-new store's payload isn't yet
    "complete", so _boot_identity returns these base instructions alone).
    Key on the NAME instead: only a store literally named "Atlas" (Pete's)
    renders "You are Atlas" — byte-identical for him; everyone else is
    themselves.
    """
    display = (name or "your agent").strip()
    if display.lower() == "atlas":
        return SYSTEM_INSTRUCTIONS
    # First-letter-capitalize so a lowercase personality ("steve") reads as
    # "Steve" while a proper display name ("Onyx", "McCoy") is untouched.
    display = display[:1].upper() + display[1:]
    return SYSTEM_INSTRUCTIONS.replace(
        "You are Atlas.", f"You are {display}.", 1
    )
...
def _boot_identity(handlers: NullHandlers, base_instructions: str) -> str:
    ...
    # The agent's own display name (from identity.json) drives the boot
    # prompt ("You are <name>"), never the personality — a fresh install has
    # personality "atlas" by back-compat default but a generated name, and
    # must introduce itself as ITS agent, not as Atlas.
    _identity = getattr(handlers.memory, "identity", None) or {}
    _name = (_identity.get("name") or personality) if isinstance(_identity, dict) else personality

    t0 = time.time()
    payload = build_identity_payload(conn, personality=personality, name=_name)
...
def create_server(agent_dir: str = "") -> tuple[FastMCP, NullHandlers]:
    ...
    # Base instructions carry the store's OWN name (from identity.json), not
    # a hardcoded 'atlas' and not the personality (which is 'atlas' by
    # back-compat default even for a fresh install with a generated name).
    # This is the cold-start path a brand-new user hits before their store
    # has enough to build a complete payload — it must say their name (H6).
    _identity = getattr(handlers.memory, "identity", None) or {}
    _boot_name = (_identity.get("name") if isinstance(_identity, dict) else None) \
        or NullHandlers._infer_personality(agent_dir)
    base_instructions = instructions_for_personality(_boot_name)
    ...
    # Report Null's own version in the MCP handshake, not the mcp SDK's.
    # FastMCP takes no version arg and leaves the lowlevel server version
    # None -> it falls back to importlib.metadata.version("mcp"), so
    # serverInfo.version disagreed with the package (issue #63).
    from null_memory.__version__ import __version__ as _null_version
    mcp._mcp_server.version = _null_version
