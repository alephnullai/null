...
def test_fresh_install_renders_its_own_name_not_atlas(populated_conn):
    """A fresh install has personality 'atlas' by back-compat default but a
    generated display name — the boot prompt MUST introduce the agent by its
    OWN name, never assert it is Atlas (Pete's one reserved persona).
    Regression: Talos caught the default install saying "You are Atlas",
    2026-07-15."""
    p = build_identity_payload(populated_conn, name="Onyx")
    assert "You are Onyx." in p.text
    assert "You are Atlas" not in p.text
    assert "ONYX IDENTITY" in p.text


def test_shipped_intro_is_generic_not_a_persons_descriptor(populated_conn):
    """The template's intro LINE must be a bare "You are <name>." — the old
    code hardcoded "You are Atlas. Pete's AI technical lead." A person's
    descriptor belongs in their OWN anchor data (which may legitimately
    mention Pete), never in the shipped template."""
    for name in ("Atlas", "Onyx", "Sage"):
        p = build_identity_payload(populated_conn, name=name)
        intro = next(l for l in p.text.splitlines() if l.startswith("You are"))
        # The intro is a bare, generic self-introduction — the ONLY place the
        # old template injected "Pete's AI technical lead". (Anchor DATA may
        # still mention Pete for Pete's own store; that's his data, not the
        # shipped template.)
        assert intro == f"You are {name}.", f"intro not generic: {intro!r}"
