...
class TestHealRegression:
    ...
    def test_cold_start_base_instructions_never_leak_atlas(self, tmp_path):
        """H6 residual (Prometheus): a BRAND-NEW pip store has personality
        'atlas' by back-compat default but a generated display name, and its
        payload isn't 'complete', so _boot_identity returns the BASE
        instructions alone. Those base instructions must say the store's own
        name, never 'You are Atlas'. This is the first-MCP-connect path a new
        user hits."""
        import os
        from null_memory.mcp.server import create_server
        os.environ.pop("NULL_PERSONALITY", None)
        _mcp, handlers = create_server(agent_dir=str(tmp_path))
        instr = (_mcp._mcp_server.instructions or "")
        name = (handlers.memory.identity or {}).get("name")
        assert name and name.lower() != "atlas"
        first = instr.splitlines()[0] if instr else ""
        assert first.startswith(f"You are {name}"), first
        assert "You are Atlas" not in first
