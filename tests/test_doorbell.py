...
def test_bind_failure_returns_false():
    ...
    try:
        # Bind to an address the host cannot own, so the bind fails
        # deterministically on every platform. 255.255.255.255 is NOT such
        # an address — Linux binds the broadcast address happily, so start()
        # returned True there. 192.0.2.1 (TEST-NET-1, RFC 5737) is not
        # assigned to any interface, so bind() fails with EADDRNOTAVAIL on
        # Linux and macOS alike.
        lst = DoorbellListener(on_ring=lambda: None,
                               bind="192.0.2.1", port=port)
