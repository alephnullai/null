...
def _snapshot_null_dir(
    root: Path | None = None, *, hash_content: bool = False,
) -> dict[str, tuple]:
    """Return {relpath: fingerprint} for every file under ``root``
    ...
    ``hash_content`` adds a content hash to each fingerprint, making the
    snapshot exact instead of timestamp-dependent. It is OFF by default and
    ON for the guard's own unit tests, and that split is deliberate:

      * WHY IT'S NEEDED. A same-size content change is invisible to
        (size, mtime_ns) whenever both writes land in the same filesystem
        timestamp tick — and on Windows that tick is ~15.6ms, so writes
        microseconds apart routinely share an mtime. That is not a corner
        case here: a git ref file (`.git/refs/heads/main`) is ALWAYS 41
        bytes, so a ref moving is precisely a same-size content change.
        The guard's unit tests rewrite such files back-to-back and were
        flaky on Windows for exactly this reason.
      * WHY IT'S NOT THE DEFAULT. This fixture is autouse — it snapshots
        the real ~/.null before AND after every one of 1600+ tests.
        Measured on that store (2004 files): hashing costs ~7s per
        snapshot, i.e. ~14s per test, hours across the suite. Unaffordable.
        On the real-store path a whole test elapses between the two
        snapshots, so mtime has long since moved and the coarse
        fingerprint is sufficient in practice.
    """
    if root is None:
        root = _REAL_NULL_DIR
    if not root.exists():
    ...
    for p in root.rglob("*"):
        ...
        try:
            st = p.stat()
            fingerprint: tuple = (st.st_size, st.st_mtime_ns)
            if hash_content:
                fingerprint += (
                    hashlib.blake2b(p.read_bytes(), digest_size=16).digest(),
                )
            out[str(p.relative_to(root))] = fingerprint
        except OSError:
            continue
    return out
