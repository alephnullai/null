...
> **The bundled `/doorbell` skill is installed automatically by `null setup`**
> (into `~/.claude/skills/doorbell/`). It ships inside the package, so a
> `pip install` seat gets it with no manual step — just run `null setup` on
> every seat. It wraps post + ring into `/doorbell <message>` and
> exception-only polling into `/doorbell check`, so agents relay work between
> seats without hand-typed CLI ceremony — and without routing through the human.

### Adaptive (dynamic) looping

When a seat watches the exchange continuously (`/loop /doorbell check` or a
scheduler), don't poll at a fixed rate — key the cadence to peer activity,
statelessly. At each wake, derive the mode from the age of the newest peer
activity (stream file mtimes and the exchange repo's last commit time; no
stored state beyond `attend`'s own cursor):

| Newest peer activity | Mode    | Next check |
|----------------------|---------|-----------:|
| < 1 h old            | hot     |     15 min |
...
nothing. The canonical spec lives in the bundled skill
([skills/doorbell/SKILL.md](../skills/doorbell/SKILL.md), "Adaptive
cadence"). Either way the transport never depends on the loop: rings
accelerate fetches, and the daemon's poll remains the delivery guarantee.
