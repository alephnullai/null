...
> **HIGHLY recommended:** install the bundled [`/doorbell` skill](../skills/doorbell/SKILL.md)
> on every seat (`cp -r skills/doorbell ~/.claude/skills/doorbell`). It wraps
> post + ring into `/doorbell <message>` and exception-only polling into
> `/doorbell check`, so agents relay work between seats without hand-typed
> CLI ceremony — and without routing through the human.
