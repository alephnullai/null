...
- **`force_rmtree` for git-repo stores.** Git writes loose objects read-only (mode 444); Windows refuses to delete read-only files (WinError 5), so `shutil.rmtree` failed on any directory containing a `.git/objects` tree. POSIX never noticed (unlink is governed by the parent dir's write bit). New `null_memory.fsutil.force_rmtree` clears the read-only bit and retries. This fixes a **live user-facing bug** — `multiverse` personality deletion raised `PermissionError` on Windows — and a silent `selftest` store leak into TEMP.
- **Watchdog now uses `perf_counter`, not `monotonic`.** On Windows `time.monotonic()` is `GetTickCount64()` at ~15.6ms resolution, so a tool call finishing inside one tick measured as exactly `0.0` and its soft-budget breadcrumb was dropped (~40% of short calls). `perf_counter` is high-resolution and matches the rest of the codebase.
- **Commit path now owns repo readiness (no longer races the deferred init thread).** `MemoryRepo.ensure_repo()` holds the init lock across the `is_repo()` check *and* `git init`, so a concurrent caller waits for a half-built repo instead of walking into it; and a commit now waits out a peer's `.git/index.lock` against a generous deadline rather than barging ahead while it's held (which, on the ~5×-slower Windows runner, made ordinary contention read as `committed=False`). Fixes the intermittent `close()`/`end_session` commit failures.
- **`selftest` teardown reaps the server and releases handles before deleting its throwaway store** (process-tree kill + reader-thread join + pipe close), with a retry-with-backoff on cleanup — closing the residual Windows store-leak that `force_rmtree`'s read-only fix didn't cover.
- **Sandbox snapshot is content-aware** (weak hash of watched files), so a same-size write — e.g. a 41-byte git ref moving — is detected regardless of the filesystem's mtime resolution (Windows collides same-size writes into one ~15.6ms tick).

Windows CI (windows-latest 3.11) is green — verified across looped re-runs, not one-shot.

## [2.2.3] — 2026-07-14

### Fixed
- **Two platform-portability test bugs** that only surfaced once CI actually ran the suite to completion (previously masked by the OOM/identity failures):
  - The doorbell bind-failure test bound to `255.255.255.255` to force a failure; Linux binds the broadcast address happily, so the test failed there. Now binds an unassignable TEST-NET address (`192.0.2.1`), which fails deterministically on every platform.
  - The prompt-verify hook's live verifiers resolved their repo root from a default argument bound at import, so a reassigned root was ignored — the test passed on dev checkouts (real phase history) but failed on an orphan release snapshot (none). The root is now resolved at call time.
